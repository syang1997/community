...
function getQuestions(num) {
    $.ajax({
        ...
        success:function (result) {
            if(result.code==0){
                addQuestionToindex(result);
            }
        }
    })
}

function searchQuestions(num) {
        search=$('#search').val();
    $.ajax({
        url:"/question/search",
        type:"POST",
        data:{"search":search,"pn":num},
        success:function (result) {
            if(result.code==0&&result.prompt!='0000'){
                    ...
                    if(pageInfo.isFirstPage){
                        ...
                        li= '                        <li>\n' +
                            '                            <a href="#" onclick="searchQuestions(1)"aria-label="Previous">\n' +
                            '                                <span aria-hidden="true">&laquo;</span>\n' +
                            '                            </a>\n' +
                            '                        </li>';
                    }
...
function addQuestionToindex(result){
    var question=result.data.list;
    var pageInfo=result.data;
    var num=1;
    var li='';
